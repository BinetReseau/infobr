...
\textbf{Sous Windows XP :} & \textbf{Sous Windows Vista :} \\

Va dans le \menu{Menu Démarrer},
\menu{Panneau de configuration} et double-clique sur
\menu{Connexions réseau} puis sur \menu{Connexion au réseau local}.
Clique enfin sur \menu{Propriétés}. &

Va dans le \menu{Menu Démarrer}, \menu{Panneau de configuration},
\menu{Réseau et Internet}, \menu{Centre réseau et partage}.
Là, dans le menu à gauche, tu cliques sur \menu{Gérer les connexions réseau},
puis clique droit sur \menu{Connexion au réseau local}, et enfin
\menu{Propriétés}

\footnote{A ce stade, ainsi qu'à plusieurs autres étapes de ce tutoriel,
Windows Vista doit normalement t'afficher un message te demandant
de confirmer l'action que tu viens d'effectuer.
Donc, tu confirmes, et cela à chaque fois !}.
...
Soit tu choisis de te mettre sur le domaine Windows, et tu vas
alors au paragraphe <<~Inscription sur le domaine Windows~>>.
...
Soit tu choisis de configurer toi-même ton ordinateur,
et tu peux passer directement à la section SECTION INSTALL ANTI-VIRUS.
Tu trouveras les informations nécessaires à la confiration manuelle du
pare-feu et du proxy pour \app{Windows Update} sur le WikiBR, à l'article
ARTICLE_QUI_VA_BIEN.
...
\app{Windows Update}. Réfère-toi pour cela à l'article ARTICLE_QUI_VA_BIEN
sur le WikiBR.
...
Clique sur \menu{Menu Démarrer}, puis fais un clic-droit sur
\menu{Ordinateur}, \menu{Propriétés}. Là tu sélectionnes
\menu{Paramètres système avancés}, onglet \menu{Nom de l'ordinateur},
puis tu cliques sur le bouton \menu{Modifier}.
\end{tabular}

Dans la case \menu{Nom de l'ordinateur}, rentre ton
pseudo, puis coche la case \menu{domaine} et rentre
\url{windows.eleves.polytechnique.fr}. Note bien que l'inscription au
domaine te sera refusée par le serveur si quelqu'un d'autre utilise
le même nom d'ordinateur que toi. Par conséquent, essaie d'utiliser un
pseudo qui t'identifie de façon claire et unique, par exemple ``NOM_PRENOM''
...
Antivirus 10.2}.}
pour lequel tu n'auras pas à payer la license pour obtenir les mises
à jour. Bien sûr, libre à toi d'utiliser ton anti-virus personnel ;
cependant il sera à ta charge de le mettre à jour très réguliérement.
...
\item[Si tu as Windows Vista...]
\begin{itemize}
\item ...en version 32 bits : \url{ftp://enez/antivirus/Win32_Vista.zip}.
\item ...en version 64 bits : \url{ftp://enez/antivirus/Win64_Vista.zip}.
\end{itemize}

\item[Si tu as Windows XP ou une autre version de Windows...]
\begin{itemize}
\item ...en version 32 bits : \url{ftp://enez/antivirus/Win32_PasVista.zip}.
\item ...en version 64 bits : \url{ftp://enez/antivirus/Win64_PasVista.zip}.
\end{itemize}
\end{description}

Une fois téléchargé le fichier adéquat, tu ouvres l'archive compressée
\file{Win??_*.zip}, tu décompresses \empf{tous les fichiers} dans un dossier,
et enfin tu exécutes le fichier \file{setup.exe} qui vient d'apparaitre
dans ce dossier. A partir de là, tu n'as plus qu'à suivre les instructions
du programme d'installation. Alors, tu n'auras plus à toucher à l'antivirus,
...
Parmi les plus simples on trouve \app{FileZilla Server} et \app{GuildFTP},
qui sont libres de surcroît. Expliquer les détails de la
configuration est un peu long pour l'InfoBR, mais tu trouveras sur
le WikiBR un tutoriel pour configurer \app{FileZilla Server} :
\url{http://frankiz/binets/reseau/Configurer_un_serveur_FTP_sous_FileZilla_Server}.
...
\begin{tabular}{p{0.5\textwidth}|p{0.5\textwidth}}
\textbf{Pour Windows XP :} & \textbf{Pour Windows Vista :} \\

L'image iso à télécharger est
\url{ftp://enez/msdnaa/win_xp/french/win_xp_pro_sp2_fr.iso}. &

Là, il y a deux possibilités :
\begin{description}
\item[Si tu as un processeur 64 bits :] L'image à télécharger est
\url{ftp://enez/msdnaa/win_vista/dvd_64bits/french/vista_dvd_x64_fr.iso}.
Attention, cette images iso est à graver sur un DVD.
\item[Si tu as un processeur 32 bits :] Là, tu peux choisir de graver :
\begin{itemize}
\item soit un DVD :
\url{ftp://enez/msdnaa/win_vista/dvd_32bits/french/vista_dvd_x86_fr.iso}.
\item soit quatre CDs (!!) :
\url{ftp://enez/msdnaa/win_vista/cd_32bits/french/vista_cd\[N\]_x86_fr.iso},
N variant de 1 à 4.
