...
Dans la case \menu{Nom de l'ordinateur}, rentre ton pseudo, puis coche la case \menu{domaine} et rentre \url{windows.eleves.polytechnique.fr}. Note bien que l'inscription au domaine te sera refusée par le serveur si quelqu'un d'autre utilise le même nom d'ordinateur que toi. Par conséquent, essaie d'utiliser un pseudo qui t'identifie de façon claire et unique, par exemple ``$NOM_PRENOM$'' \footnote{Les Jean Dupont et les Julien Thomas sont priés de trouver autre chose ;-)}.
...
%pas sur le domaine. Fais \menu{Démarrer}, \menu{Exécuter}, puis tape ``cmd'' dans la fenêtre qui s'affiche. Une ligne de %commande apparait, il te
