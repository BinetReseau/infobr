...
La configuration est d\'etaill\'ee pour Mac OS 10.8 (\app{Mountain Lion}), elle est sensiblement identique sur Mac OS 10.7 (\app{Lion}), Mac OS 10.6 (\app{Snow Leopard}) et Mac OS 10.5 (\app{Leopard}).
...
\app{Mountain Lion} : Dans la colonne de gauche, s\'electionne \menu{Ethernet}.
...
Configuration du \emph{proxy} : va dans l'onglet \menu{Proxys} et rempli l'URL comme dans la capture d'\'ecran. N'oublie pas d'activer le mode passif pour les transferts en FTP, en cochant la case comme dans la capture.
