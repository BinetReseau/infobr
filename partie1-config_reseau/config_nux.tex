...
export SYNC=rsync://rsync/gentoo-portage}
...
La solution la plus simple pour se faire un firewall sous linux est d'utiliser les iptables. Pour ceci la première étape est d'installé le paquet iptables pour ta distribution. Ensuite, la configuration peut paraître délicate mais en fait ce ne sont que des règles simples, il suffit de comprendre le principe.
...
\begin{itemize}
	\item[-A] défini la chaine à filtrer. La chaine indique d'où provient la requête : INPUT pour un paquet entrant, OUTPUT pour un paquet sortant sont les principales.
	\item[-i] défini l'interface réseau... en gros la carte réseau (eth0 le plus souvent, mais il y a aussi lo pour la boucle locale - localhost).
	...
	\item[-m] défini un module particulier à utiliser pour filtrer plus précisemment. Le plus souvent on utilise les modules tcp, udp ou state.
...
:INPUT DROP [0:0]\\
:FORWARD DROP [0:0]\\
:OUTPUT ACCEPT [14913241:2845420091]\\
\\
...
-A INPUT -p tcp -m multiport --dports 20,21,22,80,119,5050\\
\\
...
\cmdline{iptables-restore iptbl}
