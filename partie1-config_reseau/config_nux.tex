...
Le BR te conseille d'utiliser \app{FireFox}, qui est maintenant tout à fait opérationnel. Il est beaucoup plus stable que \app{Konqueror} --- le navigateur fourni par défaut avec KDE --- surtout sur les distributions réputées instables. Par contre \app{Konqueror} bénéficie d'un pré-chargement pour démarrer plus vite.

\image{partie1-config_reseau/nux_proxy_firefox}{0.6}{Configuration du proxy sous FireFox}

Dans tous les cas, la seule configuration à mettre est celle du proxy. À l'adresse \url{http://frankiz/proxy.pac} se trouve un script de configuration automatique du proxy, écrit par le BR. Dans \app{FireFox}, il faut aller dans \menu{Edit}, \menu{Preferences} et dans l'onglet \menu{General} cliquer sur \menu{Connection Settings} ; ensuite tu coches la case \menu{Configuration automatique de proxy} et tu rentres cette URL.
...
Utilise les paramètres suivants pour le configurer :
\begin{description}
  \item[Envoi du courrier] \server{poly}, pas de SSL et port 25 (port par défaut)
  \item[Réception du courrier] \server{poly}, SSL, serveur de type POP3 et port 995 (port par défaut).
  \item[login, password] il t'ont été fournis par la DSI à ton arrivée.
\end{description}

Sous \app{Knode}, il faut aussi aller dans l'onglet \menu{Extras}, et dans le cadre \menu{Encryption} pour cocher uniquement la case \menu{Use SSL}.

\emph{la, je suis pas sur du tout... celui qui relira peut confirmer cette phrase ? pkoi il parle de Knode ???? mYk.}
...
Sous \app{Knode}, c'est dans le menu \menu{Configuration} (\menu{Settings}), puis \menu{Configurer Knode}. Pour accéder aux newsgroups, il faudra choisir \server{frankiz} comme nom de serveur et le port est le port par défaut, le 119. De plus, les fora mondiaux \ngname{Usenet} sont accessibles via le serveur \server{polynews} de la DSI.

Tu peux mettre ton identité/signature par défaut pour tous les serveurs auxquels tu t'abonneras (certains élèves hébergent aussi des fora de discussion privés ou non), et chaque serveur peut bénéficier d'une identité particulière, qui peut même être le résultat d'un programme ou d'un script (par exemple pour une signature aléatoire). Les signatures commencent généralement par :
\verb*+-- +\P\ c'est à dire --<espace><retour à la ligne> de manière à ce que les \og bons \fg clients news l'interprètent comme telle, la mettent par exemple en grisé et évitent de la recopier lorsque tu réponds à un message.


Dans l'onglet \menu{Comptes} (\menu{Accounts}) et l'onglet \menu{News}, tu peux choisir le(s) serveur(s) que tu veux consulter. Enfin, pour que \app{Knode} puisse envoyer des mails, il est bon aussi d'aller dans l'onglet \menu{Mail}, et de mettre le serveur \server{poly}, port 25.
...
Pour se connecter à un serveur de news en SSL sous \app{Knode} --- par exemple à \server{ssl.polytechnique.org}, il y a une petite subtilité car il ne gère pas le SSL. Il faut installer "stunnel" pour mettre une redirection SSL de port. Dans \file{/etc/stunnel.conf} ou \file{/etc/stunnel/stunnel.conf} selon ta distribution, mets les lignes suivantes (les trois premières y sont en principe déjà) :
