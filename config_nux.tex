...
Tu dois d'abord configurer ta Gentoo pour utiliser le réseau de l'X. Configure le réseau en statique avec ton adresse IP calculée précédemment ; 
puis configure la résolution DNS : tu dois avoir ceci dans le fichier \file{/etc/resolv.conf} :
\noindent \cmdline{
...
\urllink{ftp://miroir} étant interne, il sera toujours beaucoup plus rapide que les autres.
...
configuration à effectuer est celle du serveur mandataire.

Pour \app{Firefox}, il suffit d'aller dans \menu{\'Edition}, \menu{Préférences} et dans l'onglet \menu{Avancé}, puis de cliquer sur l'onglet
\menu{Réseau}, et enfin \menu{Paramètres de connexion} ; ensuite coche la case \menu{Adresse de configuration automatique du proxy}, et rentre 
...
utilisés sont \app{Kmail} et \app{Thunderbird}. La configuration est semblable, quel que soit le
client utilisé.
