...
\begin{description}
  ...
  \item [ton broadcast] qui est de la forme \server{129.104.GGG.EEE} avec $EEE = 255 - DDD$.
        \textcolor{red}{(attention, $GGG = AAA$ n'est valable que dans les nouveaux caserts, le PEM et le BEM. Pour Foch, Fayolle et Maunoury ce n'est donc pas n\'ecessairement vrai que $GGG = AAA$.)}
\end{description}
...
comme sur tout syst\`eme linux, de le faire \`a la main \`a partir des fichiers de configuration dans \rep{/etc/}.
...
\begin{tabular}{@{\bf }l@{\xleaders\copy\point\ecart}l}
       Nom d'hôte (non obligatoire) & ton\_pseudo \\
       Serveur DNS 1       & \server{129.104.201.53} \\
       Serveur DNS 2       & \server{129.104.201.54} \\
       Serveur DNS 3       & \server{129.104.201.51} \\
       Domaine recherch\'e & eleves.polytechnique.fr polytechnique.fr \\
       Passerelle          & \server{129.104.GGG.CCC} (cf. page \pageref{calcul_ip}) \\
       P\'eriph\'erique passerelle  & aucun  \\
\end{tabular}

Clique ensuite deux fois sur suivant. (Tu n'as pas besoin d'entrer de nom d'h\^ote ZeroConf.)
Là, tu choisis l'option \menu{Oui}, puis tu valides et tu as normalement un message
qui t'annonce que ta configuration r\'eseau est termin\'ee.
Tu peux la tester en pinguant frankiz, qui doit donner quelque chose comme :
...
5050, 5053 et 5055 pour qRezix, ...) dans ton firewall pour pouvoir pleinement profiter du r\'eseau.

{\bf Configuration du gestionnaire de paquets :}
Le BR poss\`ede un miroir PLF, que tu peux utiliser en choisissant
l'onglet \menu{Gestionnaire de logiciels} dans l'utilitaire \app{drakconf}
puis en cliquant sur \menu{ajouter la source personalis\'ee}, \`a choisir \menu{Serveur FTP}
et en mettant comme url \url{ftp://miroir/plf/mandrake/free/2006.0/}
pour le premier miroir et \url{ftp://miroir/plf/mandrake/non-free/2006.0/}
pour le second. Il te faut aussi sp\'ecifier le chemin vers le
hdlist.cz, en mettant dans les deux cas ./hdlist.cz dans le champ
\menu{Chemin relatif vers hdlist/synthesis}. Enfin, n'oublie pas de
désactiver le proxy pour les utiliser.


\paragraph{\distrib{Sous Gentoo}}

Si tu n'as jamais fait de configuration r\'eseau sur ta Gentoo,
il faudra certainement cr\'eer les fichiers qui suivent.
...
Bien sûr, remplaces \texttt{eth0} par le nom de l'interface r\'eseau que tu utilises,
si ce n'est pas la carte \textit{ethernet} par d\'efaut.
...
\imagepos{images/nux_proxy_firefox}{0.66}{Configuration du proxy sous Firefox}{ht}

\flimage{images/nux_firefox_icon}{0.12}{l}
Le BR te conseille d'utiliser \app{Firefox}, ou \app{Konqueror} (le navigateur fourni par d\'efaut avec KDE).
Dans tous les cas, la seule configuration \`a mettre est celle du proxy.
Pour \app{Firefox}, il suffit d'aller dans \menu{Edit}, \menu{Preferences} et dans l'onglet \menu{General}
cliquer sur \menu{Connection Settings} ; ensuite tu coches la case
\menu{D\'etection automatique du proxy pour ce r\'eseau}, et c'est bon.
...
\flimage{images/nux_kmail_icon}{0.12}{l}
Le client mail le plus utilis\'e est \app{Kmail}, mais il en existe bien s\^ur d'autres comme \app{Thunderbird}.
La configuration est semblable, quel que soit le client que tu utilises.
...
\imagepos{images/nux_config_kmail_pop}{0.60}{Configuration de la r\'eception des messages sous Kmail}{pht}
...
\imagepos{images/nux_config_kmail_smtp}{0.60}{Configuration de l'envoi des messages sous Kmail}{pht}
...
\imagepos{images/nux_config_knode}{0.7}{Configuration de Knode}{ht}
