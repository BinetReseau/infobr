...
Pour configurer le Wifi au Bataclan, d'abord télécharger le certificat sur \urllink{http://wifi/certificats.html}.
...
\app{Safari}, le navigateur web d'Apple, est maintenant compatible avec la majorité des sites web. Tu peux donc t'en servir au quotidien, en faisant appel à \app{Firefox} ou \app{Camino} pour les sites récalcitrants. Un conseil : pense à activer le blocage des fenêtres pop-up (dans le menu \menu{Safari}). \app{Safari} peut aussi servir de client RSS (voir plus bas).\\

%\pagebreak

\subsubsection{Configuration mail}
\flimage{images/mac_mail_icone}{0.07}{r} \app{Mail} : un client mail offrant les fonctionnalités classiques d'un bon client : recherche instantanée, filtre antispam, règles de tri automatique des mails, regroupement des mails correspondant à une même discussion.
...
Mais vérifie tout de même que tu as bien un processeur Intel (\menu{menu Pomme} puis sélectionne \menu{À propos de ce Mac}).
