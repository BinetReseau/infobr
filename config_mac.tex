...
Un point particulier pour la configuration du \emph{proxy} de \app{Firefox} : dans \menu{Préférences}, \menu{Avancé}, \menu{Réseau}, clique sur \menu{Paramètres} et choisis \emph{Adresse de configuration automatique du proxy} : \urllink{http://frankiz/proxy.pac}.

\flimage{images/mac_safari_icone}{0.07}{l}
\app{Safari}, le navigateur web d'Apple, est maintenant compatible avec la majorité des sites \emph{web}. Tu peux donc t'en servir au quotidien, en faisant appel à \app{Firefox} pour les sites récalcitrants. Un conseil : pense à activer le blocage des fenêtres \emph{pop-up} (dans le menu \menu{Safari}). \app{Safari} peut aussi servir de client RSS (voir plus bas).\\
...
\flimage{images/mac_qrezix_icone}{0.07}{l} \app{qRezix} : en deux mots, c'est un programme développé par le BR pour faciliter la vie sur le réseau. Tu peux le récupérer dans la partie Mac de \xshare. Pour plus de détails, voir le paragraphe consacré à qRezix à la page \pageref{qrezix}.
