...
\begin{description}
        \item[kuzh] (\server{129.104.247.2}) : proxy http (pour l'Internet), proxy ssh pour sortir du platau, futur proxy ftp
        \item[sil] (\server{129.104.247.3}) : ancien proxy ssh double sens, proxy ftp, il va bient\^ot \^etre d\'esactiv\'e donc ne t'inqui\`ete pas si tu ne peux plus t'y connecter : econtacte un BRman ou va voir sur le WikiBR pour savoir comment faire autrement. 
        \item[poly] (\server{129.104.247.5}) : mails (réception et envoi), tu trouveras en t'y connectant en http (\url{http://poly/}) le certificat de sécurité pour l'authentification sécurisée.
...
\begin{minipage}{0.9\textwidth}
  ...
  stockage. Seul \server{sil} est prévu pour du transfert de fichiers. La DSI est assez vigilante,
